...
interface TaskColumnProps {
  ...
  onTimeBoxMove?: (timeStage: Task['timeStage'], direction: 'up' | 'down') => void;
  canMoveUp?: boolean;
  canMoveDown?: boolean;
    ...
    >
      ...
      {isEditing ? (
        ...
        <>
          <span 
            className={cn(
              'text-sm flex-grow cursor-text',
              task.agingStatus === 'overdue' && 'text-red-500 dark:text-red-400',
              task.agingStatus === 'warning' && 'text-yellow-600 dark:text-yellow-400',
              'dark:text-slate-200'
            )}
            onDoubleClick={() => onTaskTitleUpdate?.(task.id, task.title)}
          >
      ...
      >
        ...
        <DropdownMenu open={isMenuOpen} onOpenChange={setIsMenuOpen}>
          ...
          >
            ...
            {canMoveUp && (
              <DropdownMenuItem onClick={() => onTimeBoxMove?.(timeStage, 'up')}>
                <ArrowUpIcon className="w-4 h-4 mr-2" />
                Move Up
              </DropdownMenuItem>
            )}
            {canMoveDown && (
              <DropdownMenuItem onClick={() => onTimeBoxMove?.(timeStage, 'down')}>
        ...
        >
          <div 
            className={cn(
              ...
              isDraggingOver && isValidDropTarget && "bg-gray-50 dark:bg-slate-800/50"
            )}
          >
            {isAddingTask && (
