...
export const BannerManager: React.FC<BannerManagerProps> = ({
  ...
  const onDrop = useCallback((acceptedFiles: File[], type: 'images' | 'audio') => {
    const maxSize = type === 'images' ? 2 * 1024 * 1024 : 10 * 1024 * 1024;
    const validFiles = acceptedFiles.filter(file => file.size <= maxSize);

    if (validFiles.length < acceptedFiles.length) {
      alert(`Some files were too large. Maximum size: ${maxSize / (1024 * 1024)}MB`);
    }

    // In a real implementation, we would upload these files to storage
    // For now, we'll create object URLs
    const newFiles = validFiles.map((file, index) => ({
      url: URL.createObjectURL(file),
      name: file.name,
      order: type === 'images' ? config.images.length + index : config.audio.length + index
    }));
  ...
  const handleQuoteUpdate = (index: number, field: 'text' | 'author', value: string) => {
    setConfig(prev => ({
      ...prev,
              ...
              <div className="mt-4 space-y-2">
                {config.audio.map((audio, index) => (
                  ...
                  >
                    ...
                    <span className={theme === 'dark' ? "text-white" : undefined}>
                      {audio.name}
                    </span>
                    <button
                      onClick={() => {
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 space-y-4">
                      <div>
                        ...
                        <Textarea
                          value={quote.text}
                          onChange={(e) => handleQuoteUpdate(index, 'text', e.target.value)}
                          maxLength={150}
                      ...
                      <div>
                        ...
                        <Input
                          value={quote.author}
                          onChange={(e) => handleQuoteUpdate(index, 'author', e.target.value)}
